...
class AnalysisService:
    ...
    @staticmethod
    def save_analysis(user_email, image_filename, results, image_s3_key=''):
    ...
    @staticmethod
    def update_analysis_with_results(analysis_id, doctor_email, results, image_s3_key=''):
        ...
        try:
            ...
            update_expr = 'SET #status = :status, findings = :findings, total_findings = :total, image_dimensions = :dims, analyzed_at = :analyzed_at, analyzed_by = :doctor'
            expr_values = {
                ':status': 'analyzed',
                ':findings': findings,
            ...
            if image_s3_key:
                update_expr += ', image_s3_key = :s3key'
                expr_values[':s3key'] = image_s3_key
    ...
    @staticmethod
    def get_doctor_analyses(doctor_email):
        """
        Doktorun tüm analizlerini getir:
        - Doktorun kendi yüklediği (user_email = doctor_email)
        - Hasta gönderip doktorun analiz ettiği (analyzed_by = doctor_email, status = analyzed)
        """
        table = dynamodb_client.dynamodb.Table(Config.ANALYSES_TABLE)

        try:
            # 1. Doktorun kendi yükledikleri
            own_response = table.query(
                IndexName='UserEmailIndex',
                KeyConditionExpression='user_email = :email',
                ExpressionAttributeValues={':email': doctor_email}
            )
            own_analyses = own_response.get('Items', [])

            # 2. Hasta gönderip doktorun analiz ettikleri (scan - filtered)
            analyzed_response = table.scan(
                FilterExpression='analyzed_by = :doctor AND #st = :status',
            ...
            analyzed_analyses = analyzed_response.get('Items', [])

            # Birleştir ve duplicate'leri kaldır (id bazında)
            seen_ids = set()
            merged = []
            for item in own_analyses + analyzed_analyses:
                item_id = item.get('id')
                if item_id not in seen_ids:
                    seen_ids.add(item_id)
                    merged.append(item)

            merged = AnalysisService._convert_decimal_to_float(merged)
            merged.sort(key=lambda x: x.get('timestamp', ''), reverse=True)
            return merged
        except Exception as e:
            print(f"DynamoDB get_doctor_analyses error: {e}")
            return []
    ...
    def save_pending_analysis(user_email, filename, organization_id, doctor_email, patient_note='', image_url=''):
        ...
        analysis_data = {
            ...
            'total_findings': 0,
            'image_dimensions': {},
            'image_s3_key': image_url,  # S3 key (eski image_url alanının yerini aldı)
            'date': datetime.now().isoformat(),
            'analyzed_at': None,
            'analyzed_by': None
        }
